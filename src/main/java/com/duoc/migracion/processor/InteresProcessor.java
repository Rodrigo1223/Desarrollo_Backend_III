...
public class InteresProcessor implements ItemProcessor<Interes, Interes> {
    ...
    public Interes process(Interes item) {
        ...
        BigDecimal interesGenerado = saldoBase.multiply(tasa).setScale(2, RoundingMode.HALF_UP);

        // Asignamos la tasa y el interés generado a los atributos correspondientes de la entidad
        item.setTasa(tasa);
        item.setInteresGenerado(interesGenerado);

        // Actualizamos el saldo final con el interés calculado
        item.setSaldo(saldoBase.add(interesGenerado).setScale(2, RoundingMode.HALF_UP));

        log.debug("Cuenta procesada con éxito: id={}, tipo={}, tasa={}, interes={}, saldoFinal={}",
                item.getCuentaId(), tipo, tasa, interesGenerado, item.getSaldo());
